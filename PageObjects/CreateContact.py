...
class CreateContact(Locators):
    ...
    def create_contact(self, salutation, first_name, last_name, existing_account):
        ...
        try:
            ...
            self.utils.send_keys(By.XPATH, self.select_account_name, existing_account)
            self.utils.wait_for_seconds(2)
            self.utils.click_element(By.XPATH, self.select_account_name)
            self.utils.wait_for_seconds(2)
            self.utils.action_click(By.XPATH, self.account_option.format(existing_account))
            self.utils.wait_for_seconds(5)
    ...
    def verify_contact_has_saved(self, contact_name, existing_account):
        ...
        try:
            self.logger.info(f"Verifying if the contact '{contact_name}' is saved under account '{existing_account}'.")
            self.utils.action_click(By.XPATH, self.account_tab)
            self.utils.wait_for_seconds(2)
            self.utils.action_click(By.XPATH, self.existing_account.format(existing_account))
            self.utils.wait_for_seconds(2)
